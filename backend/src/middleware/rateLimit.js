 ...
 */
export function rateLimit({ limit, windowMs, message = 'Too many requests' } = {}) {
  ...
  return function rateLimitMiddleware(req, res, next) {
    ...
    // `trust proxy` is set in server.js, so this is the caller's address rather
    // than the load balancer's. Falling back to a constant would put every
    // client in one bucket, so an unknown address is let through instead —
    // failing open here is right: the key is still required, and throttling
    // everyone together because one address could not be read is worse.
    const key = req.ip
    if (!key) return next()

...
// A discharge summary is built from what clinicians typed — diagnosis, notes,
// medications — and from the patient's own name. Written raw into the print
// iframe (same origin as the app) any HTML in them would run, so every text
// field goes through the shared escaper.
...
<div class="sub">${esc(orgInfo.name)} &nbsp;·&nbsp; Generated ${format(new Date(),'dd MMM yyyy, hh:mm a')}</div>
<table>
<tr><td>Patient Name</td><td>${esc(getFullName(adm.patient))}</td></tr>
<tr><td>UHID</td><td>${esc(adm.patient?.mrn||'—')}</td></tr>
<tr><td>Admission #</td><td>${esc(admissionNo)}</td></tr>
<tr><td>Admission Date</td><td>${admDate}</td></tr>
<tr><td>Ward</td><td>${esc(wardName)}</td></tr>
<tr><td>Bed Number</td><td>${esc(adm.bed?.bedNumber||'—')}</td></tr>
<tr><td>Admission Type</td><td>${esc(adm.admissionType||'—')}</td></tr>
...
<div class="diag"><strong>Admission Diagnosis:</strong><br/>${esc(adm.admissionDiagnosis||'—')}</div>
${adm.chiefComplaint?`<div class="diag"><strong>Chief Complaint:</strong><br/>${esc(adm.chiefComplaint)}</div>`:''}
${adm.admissionNotes?`<div class="diag"><strong>Notes:</strong><br/>${esc(adm.admissionNotes)}</div>`:''}
...
<head><title>Discharge Summary — ${esc(getFullName(adm.patient))}</title>
<style>
...
<div class="page">
  ...
  <div class="grid2">
    <div class="field"><div class="field-label">Patient Name</div><div class="field-value">${esc(getFullName(adm.patient))}</div></div>
    <div class="field"><div class="field-label">UHID</div><div class="field-value">${esc(adm.patient?.mrn || '—')}</div></div>
    <div class="field"><div class="field-label">Age / Gender</div><div class="field-value">${patAge} / ${esc(adm.patient?.gender || '—')}</div></div>
    <div class="field"><div class="field-label">Phone</div><div class="field-value">${esc(adm.patient?.phonePrimary || '—')}</div></div>
  </div>
  ...
  <div class="grid2">
    ...
    <div class="field"><div class="field-label">Ward / Bed</div><div class="field-value">${esc(wardName)} — Bed ${esc(adm.bed?.bedNumber || '—')}</div></div>
    <div class="field"><div class="field-label">Admission Type</div><div class="field-value">${esc(adm.admissionType || '—')}</div></div>
    <div class="field"><div class="field-label">Length of Stay</div><div class="field-value">${days} days</div></div>
    <div class="field"><div class="field-label">Attending Doctor</div><div class="field-value">${esc(drName(adm.attendingDoctorName) || adm.attendingDoctor || '—')}</div></div>
  </div>

  <div class="section-title">Diagnosis</div>
  <div class="field" style="margin-bottom:8px"><div class="field-label">Admission Diagnosis</div><div class="field-value">${esc(adm.admissionDiagnosis || '—')}</div></div>
  ...
  ${adm.treatmentSummary
    ? `<div class="text-block">${esc(adm.treatmentSummary)}</div>`
  ...
  ${adm.medicationsOnDischarge
    ? `<div class="text-block">${esc(adm.medicationsOnDischarge)}</div>`
    : `<div class="text-block empty">Not recorded</div><div class="lines"><div class="line"></div><div class="line"></div></div>`}

  <div class="section-title">Discharge Instructions / Follow-Up</div>
  ${adm.followUpInstructions
    ? `<div class="text-block">${esc(adm.followUpInstructions)}</div>`
  ...
  ${adm.dischargeNotes ? `<div class="section-title">Additional Notes</div><div class="text-block">${esc(adm.dischargeNotes)}</div>` : ''}
  ...
  <div class="footer">This is a computer-generated document. &nbsp;|&nbsp; ${esc(orgInfo.name)} &nbsp;|&nbsp; Generated: ${printDate}</div>
</div>
</body></html>`)
  win.document.close()
}

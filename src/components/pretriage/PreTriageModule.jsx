...
import PatientLookup, {
  calculatePatientAge,
  getPatientFullName,
} from "@/components/common/PatientLookup";
...
function printSlip(
  s,
  orgInfo = { name: "Hospital", address: "", phone: "", email: "" },
) {
  const routedTo = formatRoutedTo(s.routedTo);
  const screenedAt = s.screenedAt
    ? format(new Date(s.screenedAt), "dd MMM yyyy, HH:mm")
    : "—";
...
function VitalCard({ label, value, unit, abnormal }) {
  return (
    <div
      className={`rounded border p-2 ${abnormal ? "bg-red-50 border-red-200" : "bg-gray-50 border-gray-200"}`}
    >
      <div className="text-[10px] text-gray-500">{label}</div>
      <div
        className={`text-sm font-semibold ${abnormal ? "text-red-600" : "text-gray-800"}`}
      >
        ...
        <div className="space-y-4 text-sm">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <VitalCard
                ...
                unit="/min"
                abnormal={isAbnormalResp(s.respiratoryRate)}
              />
              <VitalCard
                label="BP Systolic"
                value={s.bloodPressureSystolic}
                unit="mmHg"
                abnormal={false}
              />
              <VitalCard
                label="BP Diastolic"
                value={s.bloodPressureDiastolic}
                unit="mmHg"
                abnormal={false}
              />
            </div>
          ...
          <div className="grid grid-cols-2 gap-x-8">
            <div>
              ...
              <div className="space-y-0.5">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {s.height != null ? `${s.height} cm` : "—"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">BMI</span>
                  <span
                    className={`font-semibold ${s.bmi ? getBmiCategory(s.bmi).color : ""}`}
                  >
                    {s.bmi ?? "—"}
                  </span>
                </div>
            ...
            <div>
              ...
              <div className="space-y-0.5">
                <div className="flex justify-between">
                  <span className="text-gray-500">FBS</span>
                  <span className="font-medium">
                    {s.fbs != null ? `${s.fbs} mg/dL` : "—"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">PPBS</span>
                  <span className="font-medium">
                    {s.ppbs != null ? `${s.ppbs} mg/dL` : "—"}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {s.routedTo && (
              <span className="text-gray-600">
                Routed to: <strong>{formatRoutedTo(s.routedTo)}</strong>
              </span>
            )}
          </div>
        </div>
...
export default function PreTriageModule() {
  ...
  const stats = screeningsPagination.summary || {
    total: 0,
    pending: 0,
    routed: 0,
    registered: 0,
  ...
  useEffect(() => {
    getOrgSettings().then(setOrgInfo);
  }, []);
  ...
  const openEditDialog = (s) => {
    ...
    form.reset({
      ...
      temperature: cToF(s.temperature),
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (data.temperature != null && data.temperature !== "")
        payload.temperature = fToC(data.temperature); // typed °F → store °C
  ...
  if (screeningsPagination.loading && screenings.length === 0)
    return (
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500">
            Rapid assessment and routing for incoming patients
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats.routed}</div>
          </CardContent>
        </Card>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1 flex-wrap">
                        ...
                        <VitalBadge
                          ...
                          unit="%"
                          icon={
                            <span className="text-[10px] mr-0.5">SpO₂</span>
                          }
                          abnormal={isAbnormalSpo2(s.spo2)}
                        />
                        <VitalBadge
        ...
        >
          <div className="px-6 pt-5 pb-3 border-b shrink-0">
            <DialogTitle className="text-lg font-bold">
              {editingScreening
                ? "Edit Screening"
                : "Record New Pre-Triage Screening"}
            </DialogTitle>
            <DialogDescription>
              {editingScreening
                ...
                <div className="grid grid-cols-2 gap-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>
                          First Name {selectedPatient ? "(from record)" : ""}
                        </FormLabel>
                        <FormControl>
                          <Input
                            {...field}
                            readOnly={!!selectedPatient}
                            className={selectedPatient ? "bg-gray-50" : ""}
                          />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>
                          Last Name {selectedPatient ? "(from record)" : ""}
                        </FormLabel>
                        <FormControl>
                          <Input
                            {...field}
                        ...
                        <FormItem>
                          ...
                          <FormControl>
                            <div className="relative">
                              ...
                              {field.value && (
                                <span
                                  className={`absolute right-2 top-1/2 -translate-y-1/2 text-[10px] font-bold ${getBmiCategory(field.value).color}`}
                                >
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      >
                        <FormControl>
                          <SelectTrigger>
                            <SelectValue placeholder="Select Triage Area" />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                          <SelectItem value="adult_triage">
                            Adult Triage
                          </SelectItem>
                          <SelectItem value="pediatric_triage">
                            Pediatric Triage
                          </SelectItem>
                          <SelectItem value="mch_triage">MCH Triage</SelectItem>
                          <SelectItem value="emergency_triage">
                            Emergency Triage
                          </SelectItem>
                        </SelectContent>
                      </Select>

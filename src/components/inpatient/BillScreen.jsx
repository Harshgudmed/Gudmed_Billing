...
export default function BillScreen({ admission, orgInfo = {} }) {
  ...
  const printBill = () => {
    ...
    const rows = lines.map((c) =>
      `<tr><td>${escapeHtml(c.description)}</td><td>${c.serviceGroup}</td><td>${c.quantity}</td><td>${inr(c.unitPrice)}</td><td>${inr(c.lineTotal ?? (c.unitPrice * c.quantity))}</td></tr>`
    ).join('')
...
<table><tr><td><b>Patient</b></td><td>${escapeHtml(getFullName(admission?.patient))}</td><td><b>UHID</b></td><td>${escapeHtml(admission?.patient?.mrn || '—')}</td></tr></table>
...
<tr><td>Patient</td><td class="r">${escapeHtml(getFullName(admission?.patient))}</td></tr>
<tr><td>UHID</td><td class="r">${escapeHtml(admission?.patient?.mrn || '—')}</td></tr>

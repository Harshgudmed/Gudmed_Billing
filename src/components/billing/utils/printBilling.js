...
export function renderPaymentTable(payments, opts = {}) {
  ...
  const rows = list
    .map((p, i) => {
      ...
      const dateStr = when
        ? format(new Date(when), "dd MMM yyyy, hh:mm aa")
        : "—";
      ...
      const statusTag =
        isRefund && p.status && p.status !== "APPROVED"
...
export function printInvoice(bill, orgInfo, clinic, options = {}) {
  ...
  const calcSubtotal = bill.subtotal
    ...
    : displayItems.reduce(
        (a, i) =>
          a +
          (i.total || (i.qty || i.quantity || 1) * (i.amt || i.unitPrice || 0)),
        0,
      );

  let paymentList = [...(bill.payments || [])].sort(
    (a, b) =>
      new Date(b.paymentDate || b.date || new Date()) -
      new Date(a.paymentDate || a.date || new Date()),
  );
...
export function printReceipt(p, orgInfo, clinic) {
  ...
  const rxDate = p.paymentDate
    ? format(new Date(p.paymentDate), "dd MMM yyyy, hh:mm aa")
  ...
  const historicalPayments = sortedPayments.filter(
    (hist) => new Date(hist.paymentDate || hist.date) <= receiptDate,
  );
...
const LAB_DEPT = {
  ...
  timingLine:
    "Sample Collection Timing: 07:00 - 16:00 &nbsp;|&nbsp; Report Timing: As per test schedule.",
  newIdNote:
    "A new Lab ID will be issued for any sample submitted after the above registration date.",
};
...
const RADIOLOGY_DEPT = {
  ...
  timingLine:
    "Scan Timing: 07:00 - 20:00 &nbsp;|&nbsp; Report Timing: As per exam schedule.",
  newIdNote:
    "A new Order ID will be issued for any exam scheduled after the above registration date.",
};

function printDiagnosticReceipt(
  r,
  orgInfo = {},
  clinic = {},
  dept = LAB_DEPT,
  ...
  const showEmpty =
    clinic.showEmptyReceiptFields ?? orgInfo.showEmptyReceiptFields ?? true;
  const rows = (r.items || [])
    .map(
      (it, i) => `
    <tr><td>${i + 1}</td><td class="code">${esc(it.code)}</td>
    <td><strong>${esc(it.name)}</strong></td><td>${esc(it.eta || "")}</td>
    <td>${inr(it.price)}</td></tr>`,
    )
    .join("");
  const orderValue = Number(r.orderValue || 0);
  const home =
    r.homeCollection !== undefined
      ? Number(r.homeCollection)
      : Number(
  ...
  const regLines = [
    ...
    .map(
      ([k, v]) =>
        `<span class="k">${k}</span><span class="c">:</span><span class="v">${esc(v)}</span>`,
    )
    .join("");
  ...
  const visible = (fields) =>
    fields.filter(([, v]) => showEmpty || !isBlank(v));
  ...
  const cellHtml = ([l, v]) =>
    `<div class="cell"><span class="l">${l}</span><span class="v">${esc(isBlank(v) ? "NA" : v)}</span></div>`;
  const rowCount = Math.max(leftFields.length, rightFields.length);
  let gridCells = "";
  for (let i = 0; i < rowCount; i++) {
    gridCells += leftFields[i]
      ? cellHtml(leftFields[i])
      : '<div class="cell"></div>';
    gridCells += rightFields[i]
      ? cellHtml(rightFields[i])
      : '<div class="cell"></div>';
  }
...
export function printRadiologyReceipt(
  r,
  orgInfo = {},
  clinic = {},
  options = {},
...
export function printPharmacyReceipt(
  sale,
  orgInfo = {},
  clinic = {},
  options = {},
  ...
  const bySlab = Object.fromEntries(
    SLABS.map((s) => [s, { taxable: 0, tax: 0 }]),
  );
  let taxFree = 0;
  ...
  const rows = items
    .map((it, i) => {
      ...
      const expiry = it.expiryDate
        ? format(new Date(it.expiryDate), "MM/yy")
        : "";
      if (gstRate > 0) {
        const slab = SLABS.reduce(
          (closest, s) =>
            Math.abs(s - gstRate) < Math.abs(closest - gstRate) ? s : closest,
          SLABS[0],
        );
  ...
  const paid = Number(
    sale.amountPaid ?? sale.totalAmount ?? mrpTotal - discount,
  );

...
export default function BillingModule({ onBack }) {
  ...
  useEffect(() => {
    if (showPayModal) {
      const bal =
        showPayModal.balanceDue ??
        Number(showPayModal.total || 0) - Number(showPayModal.amountPaid || 0);
      ...
      if (res.success) {
        ...
        const mapped = res.data.map((inv) => {
          ...
          return {
            ...
            items: normItems,
            discount: inv.discountPercentage || 0,
            gstPct: 0,
            subtotal: inv.subtotal,
            discountAmt: inv.discountAmount,
            gstAmt: inv.taxAmount || 0,
            total: inv.totalAmount,
            amountPaid: inv.amountPaid || 0,
            balanceDue:
              inv.balanceDue ?? inv.totalAmount - (inv.amountPaid || 0),
  ...
  const fetchServices = useCallback(async () => {
    ...
    try {
      ...
      const res = await client.get("/billing", {
        params: { resource: "services", limit: BILLING_ITEMS_PER_PAGE, offset },
      });
      if (res.success) {
  ...
  const fetchClaims = useCallback(async () => {
    ...
    try {
      ...
      /* silent — the insurance module owns this data */
    } finally {
  ...
  const searchLabTests = useCallback(async (query) => {
    ...
    try {
      const res = await client.get("/laboratory", {
        params: { resource: "tests", search: query.trim(), limit: 500 },
      });
  ...
  const searchRadiologyExams = useCallback(async (query) => {
    setCatalogLoading(true);
    try {
      const res = await client.get("/radiology", {
        params: { resource: "exams", search: query.trim(), limit: 500 },
      });
      if (res.success) setRadiologyExams(res.data || []);
    } catch {
      toast.error("Failed to search radiology exams");
    } finally {
      setCatalogLoading(false);
    }
  }, []);
  ...
  const searchPharmacyDrugs = useCallback(async (query) => {
    setCatalogLoading(true);
    try {
      const res = await client.get("/pharmacy/drugs", {
        params: { search: query.trim(), limit: 500 },
      });
      if (res.success) setPharmacyDrugs(res.data || []);
    } catch {
      toast.error("Failed to search pharmacy inventory");
    } finally {
      setCatalogLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (!stored) {
      setClinic((c) => ({
        ...
        address:
          [hookOrgInfo.address, hookOrgInfo.city].filter(Boolean).join(", ") ||
          c.address,
  ...
  function addToCart(item, cat) {
    setForm((f) => {
      const ex = f.items.findIndex(
        (i) => i.name === item.name && i.cat === cat,
      );
      if (ex >= 0) {
        const items = [...f.items];
  ...
  async function saveInvoice() {
    ...
    const apiItems = form.items.map((it) => {
      ...
      const itemDiscount =
        subtotal > 0 ? (itemTotal / subtotal) * discountAmt : 0;
      // If department is Pharmacy, prices are GST-inclusive. Otherwise, tax is computed from gstPct.
      const itemTax =
        department === "Pharmacy"
          ? 0
          : Math.round(((itemTotal - itemDiscount) * (form.gstPct || 0)) / 100);
      ...
      return {
        ...
        ...(it.id && SOURCE_TYPE[it.cat]
          ? { sourceType: SOURCE_TYPE[it.cat], sourceId: it.id }
          : {}),
    ...
    try {
      if (form.patientId) {
        ...
        if (department === "Lab" && homeCharge > 0)
          apiItems.push({
            serviceName: "Home Collection Charges",
            quantity: 1,
            unitPrice: homeCharge,
            total: homeCharge,
            tax: 0,
            discount: 0,
          });
        const result = await client.post("/billing", {
          ...
          notes:
            `[${deptLabel}] ${form.notes || ""}${department === "Lab" && homeCharge > 0 ? ` [HCC:${homeCharge}]` : ""}`.trim(),
        });
        if (result.success) {
      ...
      if ((clinic.whatsappEnabled ?? true) && form.sendWhatsApp && form.phone) {
        const itemLines = form.items
          .map(
            (it) =>
              `  • ${it.name} x${it.qty} = ₹${(it.qty * it.amt).toLocaleString("en-IN")}`,
          )
          .join("\n");
        const msgText = [
    ...
    try {
      ...
      const rzp = new window.Razorpay({
        ...
        handler: async (response) => {
          ...
          if (verify.success) {
            setBills((bs) =>
              bs.map((b) => (b.id === bill.id ? { ...b, paid: true } : b)),
            );
    ...
    try {
      ...
      if (res.shortUrl) {
        try {
          ...
          toast.success(
            "Payment link generated. Share with patient via WhatsApp",
          );
        }
  ...
  async function recordPayment(bill, method, amount) {
    const bal =
      bill.balanceDue ?? Number(bill.total || 0) - Number(bill.amountPaid || 0);
    ...
    if (amt > bal + 0.01) {
      toast.error(
        `Amount cannot exceed balance ₹${bal.toLocaleString("en-IN")}`,
      );
      return;
    }
    ...
    try {
      ...
      if (newBal <= 0.009) {
        ...
        toast.success(
          `₹${amt.toLocaleString("en-IN")} received via ${method}. Balance ₹${newBal.toLocaleString("en-IN")}`,
        );
      }
    } catch {
  ...
  async function handleRefund(p) {
    ...
    const relatedRefunds = (showInvoiceModal?.payments || []).filter(
      (pay) =>
        pay.isRefund &&
    ...
    if (maxPaid <= 0) {
      toast.error("This receipt has already been fully refunded");
      return;
    }
  ...
  async function submitRefund() {
    ...
    try {
      ...
      if (res.success) {
        ...
        fetchStats();
        if (showInvoiceModal) {
          const updatedInvoice = await client.get("/billing", {
            params: { resource: "invoices", invoiceId: showInvoiceModal.dbId },
          });
          if (updatedInvoice.success && updatedInvoice.data[0]) {
            setShowInvoiceModal({
              ...showInvoiceModal,
              payments: updatedInvoice.data[0].payments,
            });
          }
        }
      } else {
  ...
  const rawCatItems =
    activeCat === "Lab"
      ? labTests.map((t) => ({
          id: t.id,
          name: t.testName,
          sub: t.testCategory || t.department || "",
          amt: Number(t.price || 0),
        }))
      : activeCat === "Radiology"
        ? radiologyExams.map((e) => ({
            id: e.id,
            name: e.examName,
            sub: e.examCategory || e.bodyPart || "",
            amt: Number(e.price || 0),
          }))
        : activeCat === "Pharmacy"
          ? pharmacyDrugs.map((d) => ({
              id: d.id,
              name: d.drugName + (d.strength ? ` ${d.strength}` : ""),
              sub: d.drugCategory || d.genericName || "",
              amt: Number(d.sellingPrice || d.mrp || 0),
  ...
  const catItems =
    activeCat === "Pharmacy"
      ...
      : rawCatItems.filter(
          (i) =>
            !catSearch ||
            i.name.toLowerCase().includes(catSearch.toLowerCase()) ||
            (i.sub || "").toLowerCase().includes(catSearch.toLowerCase()),
        );
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" onClick={() => setActiveTab("new-invoice")}>
                        ...
                        filteredBills.map((b) => (
                          <TableRow key={b.id}>
                            ...
                            <TableCell className="text-sm text-red-600">
                              {fmt(
                                b.balanceDue ?? b.total - (b.amountPaid || 0),
                              )}
                            </TableCell>
                            <TableCell className="text-sm text-gray-500">
                              {b.date}
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="flex items-start gap-2">
                    <span className="text-xs font-medium text-gray-500 whitespace-nowrap mt-1.5">
                      Department
                    </span>
                          ...
                          <input
                            ...
                            onChange={() => {
                              ...
                              if (
                                ...
                                if (
                                  !window.confirm(
                                    "Switching department will clear the current cart. Continue?",
                                  )
                                )
                                  return;
                              }
                  ...
                  {department && (
                    <div className="relative">
                      ...
                      <Input
                        ...
                        placeholder={`Search ${DEPT_LABEL[department]}...`}
                        value={catSearch}
                  ...
                  <div className="max-h-64 overflow-y-auto space-y-1">
                    {!department ? (
                      ...
                      catItems.map((it) => (
                        <div
                          key={it.id || it.name}
                          onClick={() => addToCart(it, activeCat)}
                        ...
                        {form.items.map((it, i) => (
                          ...
                          >
                            <div className="flex-1 min-w-0">
                              ...
                              <Badge
                                variant="outline"
                                className={`text-xs mt-0.5 ${CAT_META[it.cat]?.color || "bg-gray-100 text-gray-700"}`}
                              >
                                {it.cat}
                              </Badge>
                            </div>
                            <div className="flex items-center gap-1">
                              <button
                                ...
                                className="h-6 w-6 rounded border border-gray-300 flex items-center justify-center text-sm font-bold hover:bg-gray-100"
                              >
                                −
                              </button>
                      ...
                      <div className="bg-gray-50 rounded-lg p-3 space-y-2 border border-gray-200">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Input
                            ...
                            value={
                              Number.isNaN(form.discount) ? "" : form.discount
                            }
                            onChange={(e) => {
                              const v = parseFloat(e.target.value);
                              setForm((f) => ({
                                ...f,
                                discount: Number.isNaN(v) ? 0 : v,
                              }));
                            }}
                          />
                          {discountAmt > 0 && (
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Input
                            ...
                            max="100"
                            className="h-7 w-20 text-sm text-center"
                            value={Number.isNaN(form.gstPct) ? "" : form.gstPct}
                            onChange={(e) => {
                              const v = parseFloat(e.target.value);
                              setForm((f) => ({
                                ...f,
                                gstPct: Number.isNaN(v) ? 0 : v,
                              }));
                            }}
                          />
                          {gstAmt > 0 && (
                        ...
                        {department === "Lab" && (
                          <div className="flex items-center gap-2">
                            ...
                            <Input
                              ...
                              placeholder={String(
                                orgInfo.homeCollectionCharge || 0,
                              )}
                              value={form.homeCollection}
                              onChange={(e) =>
                                setForm((f) => ({
                                  ...f,
                                  homeCollection: e.target.value,
                                }))
                              }
                            />
                            {homeCharge > 0 && (
        ...
        <TabsContent value="insurance" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Track and manage insurance claims and reimbursements
              </CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  {claimsLoading ? (
                    ...
                    <TableRow>
                      <TableCell
                        ...
                        className="text-center py-8 text-gray-500"
                      >
                        No insurance claims yet. Add a policy and raise a claim
                        from the Insurance module.
                      </TableCell>
                    </TableRow>
                  ) : (
                    claims.map((c) => (
                      <TableRow key={c.id}>
                        ...
                        <TableCell className="text-right font-semibold text-green-700">
                          {c.approved != null ? fmt(c.approved) : "—"}
                        </TableCell>
                        <TableCell className="text-sm text-gray-500">
                          {c.submitted
                            ? format(new Date(c.submitted), "dd MMM yyyy")
                            : "—"}
                        </TableCell>
                        <TableCell>
                          <Badge
                            className={
                              CLAIM_STATUS_STYLE[c.status] ||
      ...
      <Dialog
        ...
        onOpenChange={() => setShowInvoiceModal(null)}
      >
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Invoice — {showInvoiceModal?.invoiceNo}</DialogTitle>
          </DialogHeader>
          {showInvoiceModal && (
            <div className="space-y-4 text-sm">
              <div className="grid grid-cols-2 gap-2 bg-gray-50 rounded-lg p-3">
                ...
                {showInvoiceModal.phone && (
                  <div>
                    <span className="text-gray-500">Phone: </span>
                    <span>{showInvoiceModal.phone}</span>
                  </div>
                )}
                {showInvoiceModal.uhid && (
                  <div>
                    <span className="text-gray-500">UHID: </span>
                    <span className="font-mono">{showInvoiceModal.uhid}</span>
                  </div>
                )}
                <div>
                  <span className="text-gray-500">Status: </span>
                  <PayBadge invoice={showInvoiceModal} />
                </div>
              </div>
              {(() => {
                const hccTag = String(showInvoiceModal.notes || "").match(
                  /\[HCC:(\d+(?:\.\d+)?)\]/,
                );
                const hcc = hccTag ? Number(hccTag[1]) : 0;
                const displayItems = (showInvoiceModal.items || []).filter(
                  (it) => !/home collection/i.test(it.name || ""),
                );
                const displaySubtotal = displayItems.reduce(
                  (s, it) => s + (it.amt || 0) * (it.qty || 1),
                  0,
                );
                    ...
                    <div className="flex flex-col items-end gap-4">
                      <div className="w-64 space-y-1 border rounded-lg p-3">
                        ...
                        <div className="flex justify-between font-bold text-base border-t pt-1 mt-1">
                          <span>Total</span>
                          <span>{fmt(showInvoiceModal.total)}</span>
                        </div>
                        <div className="flex justify-between text-green-700 text-sm">
                          <span>Amount Paid</span>
                          <span>{fmt(showInvoiceModal.amountPaid || 0)}</span>
                        </div>
                        <div className="flex justify-between font-semibold text-red-600 text-sm">
                          <span>Balance Due</span>
                          <span>
                            {fmt(
                              showInvoiceModal.balanceDue ??
                                showInvoiceModal.total -
                                  (showInvoiceModal.amountPaid || 0),
                            )}
                          </span>
                        </div>
                      </div>
          ...
          <DialogFooter className="gap-2">
            {showInvoiceModal &&
              (!showInvoiceModal.paid ||
                (showInvoiceModal.balanceDue ??
                  showInvoiceModal.total - (showInvoiceModal.amountPaid || 0)) >
                  0) && (
                <Button
                  onClick={() => {
                    setShowPayModal(showInvoiceModal);
                    setPayMethod("Cash");
            ...
            {(() => {
              const doPrint = (format) => {
                ...
                if (/^lab/i.test(b.department || "")) {
                  ...
                  const hccTag = String(b.notes || "").match(
                    /\[HCC:(\d+(?:\.\d+)?)\]/,
                  );
                  const hcc = hccTag ? Number(hccTag[1]) : 0;
                  const testItems = (b.items || []).filter(
                    (it) => !/home collection/i.test(it.name || ""),
                  );
                  ...
                  printLabReceipt(
                    {
                      ...
                      uhid: b.uhid,
                      age: b.age ? `${b.age} year(s)` : "",
                      sex: b.gender,
                      contact: b.phone,
                      dateTime: b.date,
                      refDoctor: b.refDoctor || "self",
                      mode: b.payMode,
                      items: testItems.map((it) => ({
                        code: (it.name || "TEST").substring(0, 6).toUpperCase(),
                        name: it.name,
                        price: (it.amt || 0) * (it.qty || 1),
                        eta: "",
                      })),
                      orderValue: testTotal,
                      homeCollection: hcc,
                      discount: b.discountAmt || 0,
                      netPayable: b.total,
                      paid: b.amountPaid || 0,
                      balance: b.balanceDue ?? b.total - (b.amountPaid || 0),
                      payments: b.payments,
                    },
                    orgInfo,
                    clinic,
                  ...
                  printRadiologyReceipt(
                    {
                      ...
                      refDoctor: b.refDoctor || "self",
                      mode: b.payMode,
                      items: examItems.map((it) => ({
                        code: (it.name || "EXAM").substring(0, 6).toUpperCase(),
                        name: it.name,
                        price: (it.amt || 0) * (it.qty || 1),
                        eta: "",
                      })),
                      orderValue: examTotal,
                      homeCollection: 0,
                      discount: b.discountAmt || 0,
                      netPayable: b.total,
                      paid: b.amountPaid || 0,
                  ...
                  printPharmacyReceipt(
                    {
                      receiptNumber: b.invoiceNo,
                      patientName: b.patientName,
                      saleDate: b.createdAt || b.date,
                      paymentMethod: b.payMode,
                      discountAmount: b.discountAmt || 0,
            ...
            <Button variant="outline" onClick={() => setShowInvoiceModal(null)}>
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
          ...
          {showPayModal && (
            <div className="space-y-4">
              ...
              {(() => {
                ...
                const balance =
                  showPayModal.balanceDue ??
                  Number(showPayModal.total || 0) - paidSoFar;
                return (
                  <div className="bg-blue-50 border border-blue-100 rounded-lg p-4">
                    <p className="font-semibold text-gray-900">
                      {showPayModal.patientName}
                    </p>
                    <p className="text-sm text-gray-500">
                      Invoice: {showPayModal.invoiceNo}
                    </p>
                    <div className="flex justify-between mt-2 text-sm">
                      <span className="text-gray-500">Total</span>
                      <span className="font-semibold">
                        {fmt(showPayModal.total)}
                      </span>
                    </div>
              ...
              <div className="border rounded-lg p-4 space-y-3">
                ...
                <Button
                  ...
                  onClick={() =>
                    showPayModal &&
                    recordPayment(showPayModal, payMethod, payAmount)
                  }
                >
                  {savingPayment
                    ? "Recording…"
                ...
                <p className="text-xs text-gray-400 text-center">
                  Tip: pay part now (e.g. ₹500 Cash), then add another method
                  for the rest — balance updates automatically.
                </p>
              </div>
            </div>
          )}
          <DialogFooter>
            <Button variant="ghost" onClick={() => setShowPayModal(null)}>
              Cancel
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog
        open={!!refundDialog}
        onOpenChange={(o) => !o && setRefundDialog(null)}
      >
            ...
            <div className="bg-gray-50 rounded-xl p-4 flex justify-between items-center border border-gray-100">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 mt-1">
                  ₹{refundDialog?.maxPaid?.toLocaleString()}
                </p>
              </div>
              <div className="text-right">
                ...
                <p className="text-sm font-medium text-gray-700 mt-1">
                  ₹{refundDialog?.amount?.toLocaleString()}
                </p>
              </div>
            </div>

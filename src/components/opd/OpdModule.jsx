      ...
      <CardHeader className="py-3 bg-gradient-to-r from-slate-50 to-white border-b border-slate-100">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span
              className={`flex h-9 w-9 items-center justify-center rounded-xl ${ring}`}
            >
              {icon}
            </span>
            <div>
...
export default function OpdModule() {
  ...
  const opdDateRange = (() => {
    ...
    if (filterDate === "week")
      return {
        startDate: ymd(new Date(Date.now() - 7 * 86400000)),
        endDate: ymd(now),
      };
    if (filterDate === "month")
      return {
  ...
  const stats = consultationsPagination.summary || {
    total: 0,
    today: 0,
    thisWeek: 0,
    withRx: 0,
  ...
  const fetchAll = useCallback(async () => {
    ...
    if (uRes.status === "fulfilled")
      setDoctors(
        (uRes.value?.data ?? []).filter(
          (u) => u.role === "doctor" && u.isActive,
        ),
      );
    if (dRes.status === "fulfilled") setDrugs(dRes.value?.data ?? []);
    if (lRes.status === "fulfilled")
      setLabTests((lRes.value?.data ?? []).filter((t) => t.isActive !== false));
    if (rRes.status === "fulfilled")
      setRadiologyExams(
        (rRes.value?.data ?? []).filter((e) => e.isActive !== false),
      );
  ...
  const onDepartmentChange = async (dept) => {
    ...
    try {
      const res = await client.get(
        `/clinical-kb?specialty=${encodeURIComponent(dept)}`,
      );
  ...
  const onProblemChange = async (cond) => {
    ...
    try {
      const res = await client.get(
        `/clinical-kb/condition?specialty=${encodeURIComponent(department)}&condition=${encodeURIComponent(cond)}`,
      );
      ...
      if (g) {
        ...
        if (g.icd10 && !/not specified/i.test(g.icd10))
          clinicalForm.setValue("icd10Code", g.icd10);
        if (!clinicalForm.getValues("chiefComplaint"))
          clinicalForm.setValue("chiefComplaint", g.condition || cond);
  ...
  const addDrug = () => {
    ...
    if (!drug) return;
    if (prescriptionItems.some((i) => i.drugId === drug.id)) {
  ...
  const addSuggestedDrug = (name) => {
    if (
      prescriptionItems.some(
        (i) => (i.drugName || "").toLowerCase() === name.toLowerCase(),
      )
    )
      return;
    const match = drugs.find(
      (d) =>
        d.drugName?.toLowerCase() === name.toLowerCase() ||
  ...
  const removeDrug = (i) =>
    setPrescriptionItems((p) => p.filter((_, idx) => idx !== i));
  ...
  const addLabTest = () => {
    ...
    if (!test) return;
    if (labOrderItems.some((i) => i.testId === test.id)) {
      toast.error("Already added");
      return;
    }
  ...
  const addSuggestedTest = (name) => {
    if (
      labOrderItems.some(
        (i) => (i.testName || "").toLowerCase() === name.toLowerCase(),
      )
    )
      return;
  ...
  const removeLabItem = (testId) =>
    setLabOrderItems((prev) => prev.filter((i) => i.testId !== testId));

  const addRadExam = () => {
    const exam = radiologyExams.find((e) => e.id === selectedRadExam);
    if (!exam) return;
    if (radiologyOrderItems.some((i) => i.examId === exam.id)) {
      toast.error("Already added");
      return;
    }
    setRadiologyOrderItems((prev) => [
      ...prev,
      {
        examId: exam.id,
  ...
  const removeRadItem = (examId) =>
    setRadiologyOrderItems((prev) => prev.filter((i) => i.examId !== examId));

  const isTestAdded = (name) =>
    labOrderItems.some(
      (i) => (i.testName || "").toLowerCase() === name.toLowerCase(),
    );
  const isDrugAdded = (name) =>
    prescriptionItems.some(
      (i) => (i.drugName || "").toLowerCase() === name.toLowerCase(),
    );
  ...
  const openEdit = (c) => {
    ...
    if (c.prescriptions && c.prescriptions.length > 0) {
      try {
        setPrescriptionItems(JSON.parse(c.prescriptions[0].items));
      } catch {
        setPrescriptionItems([]);
      }
    ...
    if (c.radiologyOrders && c.radiologyOrders.length > 0) {
      ...
      c.radiologyOrders.forEach((order) => {
        const exam =
          order.exam || radiologyExams.find((e) => e.id === order.examId);
        if (exam)
          items.push({
    ...
    clinicalForm.reset({
      ...
      icd10Code: (() => {
        try {
          const a = JSON.parse(c.icd10Codes || "[]");
      ...
      followUpDate: c.followUpDate
        ? format(new Date(c.followUpDate), "yyyy-MM-dd")
        : "",
    });
    setView("form");
  };
  ...
  const saveConsultation = async () => {
    ...
    try {
      const payload = {
        ...
        radiologyExams:
          radiologyOrderItems.length > 0 ? radiologyOrderItems : undefined,
      };
      if (editingId) {
          ...
          <div className="flex items-center justify-between flex-wrap gap-4 mb-6">
            <div>
              ...
              <p className="text-gray-500 text-sm mt-1">
                View, manage and print patient consultation records
              </p>
            </div>
            <div className="flex gap-2">
              <Button
                variant="outline"
                size="sm"
                onClick={consultationsPagination.refresh}
              >
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 min-w-[220px] max-w-sm">
            ...
            <Input
              ...
              placeholder="Search by patient, UHID, diagnosis..."
              value={filterSearch}
            ...
            <CardContent className="flex flex-col items-center py-16 text-center">
              ...
              <p className="text-gray-400 text-sm mt-1 mb-6">
                {filterSearch || filterDoctor !== "all"
                  ...
                  : "Start by creating a new OPD prescription"}
              </p>
              {!filterSearch && filterDoctor === "all" && (
                <Button
                  onClick={() => {
                  ...
                  className="bg-[#2E4168] hover:bg-[#24344f]"
                >
                  <Plus className="h-4 w-4 mr-2" />
                  New Prescription
                </Button>
          ...
          <div className="space-y-3">
            {paginated.map((c) => {
              ...
              return (
                <Card
                  key={c.id}
                  className="rounded-2xl border-slate-200/70 shadow-sm hover:shadow-md hover:border-[#2E4168]/25 transition-all"
                >
                        ...
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            ...
                            <p className="text-sm text-gray-500">
                              UHID: {patMrn} &bull; {patAge} yrs &bull;{" "}
                              {patGender}
                            </p>
                          </div>
                          <div className="text-sm text-gray-500 shrink-0">
                            {format(new Date(c.visitDate), "dd MMM yyyy HH:mm")}
                          </div>
                        </div>
                        ...
                        <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
                          ...
                          {c.followUpDate &&
                            (() => {
                              ...
                              return (
                                <span
                                  className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium border ${upcoming ? "bg-white text-green-700 border-green-300" : "bg-white text-gray-500 border-gray-200"}`}
                                >
                                  <CalendarClock className="h-3.5 w-3.5" />
                                  {upcoming
                                    ? `Follow-up: ${format(fu, "dd MMM yyyy")}`
                                    : `Follow-up was: ${format(fu, "dd MMM yyyy")}`}
                                </span>
                              );
                            })()}
                        </div>
            ...
            <Pagination
              ...
              onPageChange={consultationsPagination.setPage}
            />
          </div>
        )}
        ...
        <Dialog open={!!viewing} onOpenChange={() => setViewing(null)}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                {viewing && viewing.patient
                  ? `${getFullName(viewing.patient)} (${viewing.patient.mrn}) — ${format(new Date(viewing.visitDate), "dd MMM yyyy")}`
                  : ""}
              </DialogDescription>
            </DialogHeader>
            {viewing &&
              (() => {
                const c = viewing;
                const meds = (c.prescriptions || []).flatMap((rx) => {
                  try {
                    return JSON.parse(rx.items || "[]");
                  } catch {
                    return [];
                  }
                });
                const labs = (c.labOrders || []).flatMap((o) => {
                  try {
                ...
                const rads = (c.radiologyOrders || [])
                  .map((o) => o.exam?.examName || o.examName)
                  .filter(Boolean);
                return (
                  <div className="space-y-4 text-sm">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-[#2E4168]/5 p-3 rounded-lg">
                      <div>
                        ...
                        <p className="font-semibold">
                          {c.patient ? getFullName(c.patient) : "—"}
                        </p>
                      </div>
                      <div>
                    ...
                    <div className="bg-[#2E4168]/5 border border-[#2E4168]/25 rounded p-3">
                      <p className="font-semibold text-[#2E4168] text-xs uppercase tracking-wide mb-1">
                        Diagnosis
                      </p>
                    ...
                    {c.chiefComplaint && (
                      <div>
                        <p className="font-semibold text-gray-700 uppercase text-xs tracking-wide mb-1">
                          Complaint
                        </p>
                      ...
                      <div>
                        ...
                        <div className="border rounded divide-y">
                          {meds.map((m, i) => (
                            <div
                              key={i}
                              className="p-2 flex flex-wrap justify-between gap-2"
                            >
                              ...
                              <span className="text-gray-500 text-xs">
                                {[
                                  ...
                                  m.frequency,
                                  m.duration,
                                  m.quantity ? `Qty: ${m.quantity}` : null,
                                ]
                                  .filter(Boolean)
                                  .join(" · ")}
                              </span>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {labs.map((t, i) => (
                            <span
                              key={i}
                              className="bg-blue-50 text-blue-700 border border-blue-200 rounded px-2 py-0.5 text-xs"
                            >
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {rads.map((n, i) => (
                            <span
                              key={i}
                              className="bg-purple-50 text-purple-700 border border-purple-200 rounded px-2 py-0.5 text-xs"
                            >
                              {n}
                            </span>
                          ))}
                    ...
                    {c.followUpDate && (
                      <div className="bg-green-50 border border-green-200 rounded p-2 text-green-800 font-medium">
                        Follow-up:{" "}
              ...
              {viewing && (
                <Button
                  ...
                  onClick={() => {
                    ...
                    printPrescription(
                      c,
                      c.patient ? getFullName(c.patient) : "Unknown",
                      c.patient?.mrn || "—",
                      c.patient?.dateOfBirth
                        ? getAge(c.patient.dateOfBirth)
                        : 0,
                      c.patient?.gender || "",
                      c.doctor?.fullName || "—",
                      orgInfo,
        ...
        >
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                This permanently removes the OPD prescription
                {deleting?.patient
                  ? ` for ${getFullName(deleting.patient)}`
                  : ""}{" "}
                and its medicines &amp; test orders. This cannot be undone.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
              <Button
  ...
  const SaveBtn = ({ size }) => (
    <Button
      ...
      disabled={isSaving}
      size={size}
      className="bg-white text-[#2E4168] hover:bg-[#2E4168]/5 shadow-sm"
    >
      {isSaving ? (
        <Loader2 className="h-4 w-4 mr-2 animate-spin" />
      ) : (
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold flex items-center gap-2">
              ...
              {editingId ? "Edit OPD Prescription" : "New OPD Prescription"}
            </h1>
            <p className="text-white/75 text-xs">
              Department → Problem → add medicines &amp; tests → save
            </p>
          </div>
        </div>
        <SaveBtn />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className="xl:col-span-2 space-y-4">
          ...
          <Section
            ...
            desc="Who is this prescription for"
          >
              ...
              <div>
                ...
                {selectedPatient ? (
                  <div className="flex items-center gap-3 p-3 bg-[#2E4168]/5 rounded-xl border border-[#2E4168]/25">
                    <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-[#2E4168] to-[#3b5488] flex items-center justify-center text-white text-sm font-bold">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500">
                        UHID: {selectedPatient.mrn} &bull;{" "}
                        {getAge(selectedPatient.dateOfBirth)} yrs &bull;{" "}
          ...
          <Section
            ...
            title="Condition & Suggestions"
            desc="Diagnosis auto-fills; tap chips to add tests & medicines"
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label className="text-sm font-medium mb-2 block">
              ...
              <div>
                ...
                <SearchableSelect
                  ...
                  options={conditions.map((c) => ({
                    ...
                    label: c.condition,
                    sublabel:
                      c.icd10 && !/not specified/i.test(c.icd10)
                        ? c.icd10
                        : undefined,
                    keywords: c.icd10 || "",
                  }))}
                  placeholder={
                    department ? "Select problem" : "Select a department first"
                  }
            ...
            {guidance && (
              <div className="space-y-3 mt-4">
                {guidance.suggestedTests?.length > 0 && (
                  <div>
                    <p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-blue-700 mb-2">
                ...
                {guidance.suggestedDrugs?.length > 0 && (
                  <div>
                    <p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-green-700 mb-2">
                      <Pill className="h-3.5 w-3.5" />
                      Suggested Medicines{" "}
                      <span className="text-gray-400 font-normal normal-case">
                        — tap to add, then set dose
                      </span>
                    </p>
                    <div className="flex flex-wrap gap-2">
                ...
                {!guidance.suggestedTests?.length &&
                  !guidance.suggestedDrugs?.length && (
                    <p className="text-sm text-gray-400">
                      No structured suggestions — add medicines &amp; tests
                      manually below.
                    </p>
                  )}
                  ...
                  {guidanceOpen && (
                    <div className="px-4 pb-4">
                      <div className="flex items-center gap-2 mb-3 text-xs font-medium text-amber-700 bg-amber-50 border border-amber-200 rounded px-2.5 py-1.5">
                        ...
                        Advisory only — verify every dose before prescribing.
                      </div>
                      <div className="max-h-[360px] overflow-y-auto pr-1">
                        <GuidanceField
                          label="When to Suspect"
                          value={guidance.whenToSuspect}
                        />
                        <GuidanceField
                          label="Investigations"
                          value={guidance.investigations}
                        />
                        <GuidanceField
                          label="Drugs & Dosage"
                          value={guidance.drugs}
                        />
                        <GuidanceField
                          label="Management"
                          value={guidance.management}
                        />
                        <GuidanceField
                          label="Referral / Red Flags"
                          value={guidance.referralRedFlags}
                        />
                      </div>
                    </div>
                  )}
                      ...
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            ...
                            {...field}
                            onChange={(e) =>
                              field.onChange(f.parse(e.target.value))
                            }
                          />
                        </FormControl>
                        <FormMessage />
            ...
            <div className="space-y-4">
              <div className="flex gap-2">
                <SearchableSelect
                  ...
                  emptyText={
                    drugs.length === 0
                      ? "No drugs available — seed pharmacy first"
                      : "No matching drugs"
                  }
                  options={drugs.map((d) => ({
                    value: d.id,
              ...
              {prescriptionItems.length === 0 ? (
                <div className="text-center py-8 text-gray-400 border-2 border-dashed rounded-xl">
                  ...
                  <p className="text-sm">
                    No medicines yet — tap a suggestion above or search here
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  {prescriptionItems.map((item, i) => (
                    <div
                      key={i}
                      className="border rounded-xl p-3 bg-slate-50/70"
                    >
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                        ...
                        <div>
                          ...
                          <Select
                            ...
                            onValueChange={(v) => updateItem(i, "frequency", v)}
                          >
                        ...
                        <div>
                          ...
                          <Select
                            value={item.duration}
                            onValueChange={(v) => updateItem(i, "duration", v)}
                          >
                            <SelectTrigger className="h-8 text-sm rounded-lg">
                              <SelectValue />
                            </SelectTrigger>
              ...
              <div className="flex gap-2">
                <SearchableSelect
                  ...
                  options={labTests.map((t) => ({
                    ...
                    sublabel: [t.testCategory, t.specimenType]
                      .filter(Boolean)
                      .join(" · "),
              ...
              <div className="flex gap-2">
                <SearchableSelect
                  ...
                  options={radiologyExams.map((e) => ({
                    ...
                    sublabel: [
                      e.examCategory && e.examCategory.toUpperCase(),
                      e.bodyPart,
                      e.modality,
                    ]
                      .filter(Boolean)
              ...
              {labOrderItems.length > 0 || radiologyOrderItems.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {labOrderItems.map((item) => (
                    <span
                      key={item.testId}
                      className="inline-flex items-center gap-1.5 bg-blue-50 text-blue-700 border border-blue-200 rounded-full px-3 py-1 text-sm shadow-sm"
                    >
                      <FlaskConical className="h-3.5 w-3.5" />
                      {item.testName}
                      <button
                        type="button"
                        onClick={() => removeLabItem(item.testId)}
                  ...
                  {radiologyOrderItems.map((item) => (
                    <span
                      key={item.examId}
                      className="inline-flex items-center gap-1.5 bg-purple-50 text-purple-700 border border-purple-200 rounded-full px-3 py-1 text-sm shadow-sm"
                    >
                      <Scan className="h-3.5 w-3.5" />
                      {item.examName}
                      <button
                        type="button"
                        onClick={() => removeRadItem(item.examId)}
                ...
                <div className="text-center py-6 text-gray-400 border-2 border-dashed rounded-xl">
                  <FlaskConical className="h-7 w-7 mx-auto mb-1 opacity-40" />
                  <p className="text-sm">
                    No tests yet — tap a suggestion above or search here
                  </p>
                </div>
              )}
            </div>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="md:col-span-2">
                      <FormLabel>Advice / Instructions</FormLabel>
                      <FormControl>
                        <Textarea
                          rows={2}
                          className="rounded-xl"
                          placeholder="Diet, rest, warning signs, when to return..."
                          {...field}
                        />
                      </FormControl>
                    </FormItem>
          ...
          <div className="xl:sticky xl:top-24">
            <Card className="rounded-2xl border-slate-200/70 shadow-md overflow-hidden">
              <div className="bg-gradient-to-r from-[#2E4168] to-[#3b5488] text-white px-4 py-3 flex items-center justify-between">
                <span className="font-semibold flex items-center gap-2">
                  <span className="text-xl font-serif">℞</span> Live
                  Prescription
                </span>
                <Badge className="bg-white/20 text-white hover:bg-white/20 border-0">
                  {prescriptionItems.length} med ·{" "}
                  {labOrderItems.length + radiologyOrderItems.length} test
                </Badge>
              </div>
              <CardContent className="p-4 space-y-3 text-sm">
                ...
                <div className="rounded-xl bg-slate-50 p-3">
                  {selectedPatient ? (
                    <>
                      ...
                      <p className="text-xs text-gray-500">
                        ...
                        {getAge(selectedPatient.dateOfBirth)}y ·{" "}
                        {selectedPatient.gender}
                      </p>
                    </>
                  ) : (
                  ...
                  {selectedDoctor && (
                    <p className="text-xs text-[#2E4168] mt-1">
                      {drName(selectedDoctor.fullName)}
                    </p>
                  )}
                </div>
                ...
                {(vw.temperature ||
                  ...
                  <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-600">
                    ...
                    {vw.bloodPressureSystolic ? (
                      <span>
                        BP {vw.bloodPressureSystolic}/
                        {vw.bloodPressureDiastolic}
                      </span>
                    ) : null}
                    {vw.pulseRate ? <span>♥ {vw.pulseRate}</span> : null}
                    {vw.oxygenSaturation ? (
                      <span>SpO₂ {vw.oxygenSaturation}%</span>
                    ) : null}
                ...
                {/* Diagnosis */}
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-wide text-[#2E4168]">
                    Diagnosis
                  </p>
                ...
                {/* Medicines */}
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-wide text-green-700 flex items-center gap-1">
                    <Pill className="h-3 w-3" />
                    Medicines
                  </p>
                  {prescriptionItems.length === 0 ? (
                    <p className="text-gray-400 italic text-xs">None yet</p>
                  ) : (
                ...
                {/* Tests */}
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-wide text-blue-700 flex items-center gap-1">
                    <FlaskConical className="h-3 w-3" />
                    Tests
                  </p>
                  {labOrderItems.length + radiologyOrderItems.length === 0 ? (
                    <p className="text-gray-400 italic text-xs">None yet</p>
                  ) : (
                    <div className="flex flex-wrap gap-1 mt-1">
                      {labOrderItems.map((t) => (
                        <span
                          key={t.testId}
                          className="bg-blue-50 text-blue-700 border border-blue-200 rounded px-1.5 py-0.5 text-xs"
                        >
                          {t.testName}
                        </span>
                      ))}
                      {radiologyOrderItems.map((t) => (
                        <span
                          key={t.examId}
                          className="bg-purple-50 text-purple-700 border border-purple-200 rounded px-1.5 py-0.5 text-xs"
                        >
                          {t.examName}
                        </span>
                      ))}
                ...
                {(wAdvice || wFollow) && (
                  <div className="border-t pt-2 text-xs text-gray-600">
                    {wAdvice && (
                      <p>
                        <span className="font-semibold">Advice:</span> {wAdvice}
                      </p>
                    )}
                    {wFollow && (
                      <p className="text-green-700 font-medium mt-0.5">
                        Follow-up: {format(new Date(wFollow), "dd MMM yyyy")}
                      </p>
                    )}
                  </div>
                )}
            ...
            <div className="mt-3 hidden xl:block">
              <Button
                onClick={saveConsultation}
                disabled={isSaving}
                className="w-full bg-[#2E4168] hover:bg-[#24344f]"
                size="lg"
              >
                {isSaving ? (
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
